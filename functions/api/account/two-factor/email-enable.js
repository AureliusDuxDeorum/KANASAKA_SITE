...
import { enableEmail2fa } from "../../../lib/two-factor.js";
...
export async function onRequestPost(context) {
  ...
  if (!/^\d{6}$/.test(code.replace(/\s+/g, ""))) {
    return errorResponse("Enter the 6-digit code from your email.");
  }

  try {
    const result = await enableEmail2fa(env, user.id, code);
    await logAuthEvent(env, "twofa_enabled", { ip: clientIp(request), userId: user.id, method: "email" });
    return jsonResponse({
      success: true,
